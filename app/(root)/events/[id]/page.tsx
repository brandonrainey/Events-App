...
export default async function EventDetails({
  ...
  const realtedEventsByUser = await getEventsByUser({
    userId: event.organizer._id,
    page: Number(searchParams.page),
  })

  const allRealtedEvents = relatedEvents?.data.concat(
    realtedEventsByUser?.data.filter(
      (event: IEvent) =>
        event._id !== id &&
        !relatedEvents?.data.some((e: IEvent) => e._id === event._id)
    )
  )
      ...
      <section className="flex justify-center bg-primary-50  bg-contain">
        <div className="grid grid-cols-1 md:grid-cols-2 2xl:max-w-7xl">
          <Image
            ...
            width={1000}
            height={1000}
            className="max-h-full min-h-[300px] object-cover object-center"
          />
